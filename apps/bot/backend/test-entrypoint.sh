...
then
    ...
    if [ -d "/test/apps/bot/backend/build/reports" ]; then
        cp -r /test/apps/bot/backend/build/reports /tmp/reports/apps/bot/backend
    fi

    zip -r /tmp/report.zip /tmp/reports

    if [ "$test_status" -eq 0 ]; then
        subject="Usada pekora bot backend tests success"
    else
        subject="Usada pekora bot backend tests failed"
    fi
