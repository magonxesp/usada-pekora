...
class TriggerDefaultAudioResponseGetApiControllerTest : TriggerControllerTest() {
    ...
    fun `should find a trigger audio`() {
        ...
        request("/api/v1/trigger/response/audio/${audio.id.value}").andExpect {
    ...
    fun `should not find a trigger audio`() {
        request("/api/v1/trigger/response/audio/e322b3ac-2d30-4eff-afdc-3504f66ac4ba").andExpect {
    ...
    fun `should get the file contents of trigger audio`() {
        ...
        request("/api/v1/trigger/response/audio/$audioId/content").andExpect {
            assertEquals(200, it.response.status)
    ...
    fun `should not get the file contents of trigger audio does not exists`() {
        request("/api/v1/trigger/response/audio/e322b3ac-2d30-4eff-afdc-3504f66ac4ba/content").andExpect {
            assertEquals(404, it.response.status)
        }
    }
