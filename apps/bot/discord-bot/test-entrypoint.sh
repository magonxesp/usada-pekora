...
then
    ...
    if [ -d "/test/contexts/bot/build/reports" ]; then
        cp -r /test/contexts/bot/build/reports /tmp/reports/contexts/bot
    fi

    if [ -d "/test/apps/bot/discord-bot/build/reports" ]; then
        cp -r /test/apps/bot/discord-bot/build/reports /tmp/reports/apps/bot/discord-bot
    fi

    zip -r /tmp/report.zip /tmp/reports

    if [ "$test_status" -eq 0 ]; then
        subject="Usada pekora bot discord-bot tests success"
    else
        subject="Usada pekora bot discord-bot tests failed"
    fi
