...
class GuildMemberCreator(private val repository: GuildMemberRepository) {
    fun create(request: GuildMemberCreateRequest): Either<GuildMemberError, Unit> {
        repository.find(UserId(request.userId), GuildId(request.guildId))
            .onRight { return GuildMemberError.AlreadyExists("The user ${request.userId} is already member of guild ${request.guildId}").left() }
