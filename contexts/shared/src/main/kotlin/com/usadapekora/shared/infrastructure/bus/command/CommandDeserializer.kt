...
class CommandDeserializer(private val registry: CommandRegistry) {
	...
	fun deserialize(rawCommand: String): Command {
		...
		val commandClass = commands.singleOrNull { it.simpleName!!.camelToSnakeCase() == commandName.commandName }
			?: throw RuntimeException("Command class for command name ${commandName.commandName} not found")
